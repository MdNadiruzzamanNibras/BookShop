...
import {
  ...
  setPublicationYear,
  setSearchQuery,
} from "../redux/filterdata/filterSlice";
...
const AllBooks = () => {
  ...
  const { publicationYear, genre, searchQuery } = useAppSelector(
    (state) => state.filter
  );
  ...
  if (publicationYear) {
    filteredBooks = filteredBooks.filter(
      (book) => book.PublicationDate === publicationYear
    );
  }
      ...
      <div className="flex justify-around text-center my-10">
        ...
        <div className="form-control">
          ...
          <div className="flex items-center">
            ...
            />
            <button
              className="px-6 py-3 text-base ml-3 rounded-lg bg-black text-white "
            ...
            <tbody>
              {filteredBooks &&
                filteredBooks?.map((book: IBook, index: number) => (
                  <tr
                    className="lg:text-lg font-bold text-center"
                    key={book?._id}
                  >
                    <th>{index + 1}</th>
                    <td>{book?.Title}</td>
                    ...
                    <td>
                      <Link to={`/book/${book._id}`}>
                        <button className="bg-green-400 text-white text-base px-4 py-1 rounded-full">
                          Details
                        </button>
                      </Link>
                      <button
                        onClick={() => dispatch(addToWish(book))}
                        className="bg-black ml-4 text-white text-base px-4 py-1 rounded-full"
                      >
                        add to wishlist
                      </button>{" "}
                    </td>
